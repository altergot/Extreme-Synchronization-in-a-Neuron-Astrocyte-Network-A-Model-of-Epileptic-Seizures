function [model, Count_Iastro_neuron] = count_network_step(params,i, model, Count_Iastro_neuron)
...
arr_I_neuro(Az, 1) = params.impact_X  .* model.X(Az, 1);

Iastro_neuron = zeros(params.N_neurons,1);
...
model.IP3 = model.IP3 + params.step .* ((IP3s - model.IP3) * Tr + Iplc + arr_I_neuro) ./1000;

Az = find(model.Ca >= params.threshold_Ca);
Count_Iastro_neuron (Az,1) = 50000;
Iastro_neuron(Count_Iastro_neuron > 0,1) = 1;

model.I_EPSCs = model.I_EPSCs + params.step .*  (params.alpha_I *(params.I_EPSCs0 - model.I_EPSCs) - params.impact_Ca .* max(model.Ca,params.threshold_Ca)  .* Iastro_neuron);

S1 = 1 ./ (1 + exp((-model.V ./ params.ksyn)));
Isyn = -sum(model.A1.*S1', 2) .* model.V .* model.I_EPSCs ./ model.D;
Isyn(model.D == 0) = 0;
